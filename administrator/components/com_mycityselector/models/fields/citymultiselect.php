...
class JFormFieldCitymultiselect extends JFormFieldList
{

	protected $type = "Citymultiselect";
	...
	public function getOptions()
	{
		...
		{
			if (in_array($city['id'], $disabled))
			{
				$options[] = Joomla\CMS\HTML\HTMLHelper::_('select.option', $city['id'], $city['name'], 'value', 'text', true);
			}
			else
			{
	...
	protected function getDisabled()
	{
		...
		$db            = Joomla\CMS\Factory::getDbo();
		$query         = $db->getQuery(true);
		$queryText     = 'SELECT DISTINCT city_id FROM #__mycityselector_field_value AS a
			LEFT JOIN #__mycityselector_value_city AS b ON a.id = b.field_value_id
	...
	protected function getSelected()
	{
		...
		$db            = Joomla\CMS\Factory::getDbo();
		$query         = $db->getQuery(true);
		$query->select('city_id')->from('#__mycityselector_value_city')
