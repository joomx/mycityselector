...
class JFormFieldCountrynames extends JFormField
{
     ...
     protected function getEditForm($langs)
     {
        $html = '';
         ...
         foreach ($langs as $lang) {
             ...
             $country = $this->getCountryNameByCountryIdAndByLangId($_GET['id'], $lang['id']);
             ...
             if(empty($country)) {
                 ...
                 $html .= "<label for=\"name{$lang['id']}\">{$lang['locale']}: </label><input id=\"name{$lang['id']}\" type='text' name=\"jform[name][{$lang['id']}]\" class=\"form-control\" placeholder='Name' value=\"{$country[0]['name']}\"><br><br>";
             }
         }
    ...
    protected function getCreateForm($langs)
    {
        ...
        foreach ($langs as $lang) {
            $html .= "<label for=\"name{$lang['id']}\">{$lang['locale']}: </label><input id=\"name{$lang['id']}\" type='text' name=\"jform[name][{$lang['id']}]\" placeholder='Name'><br><br>";
        }

        return $html;
    }

     protected function getCountryNameByCountryIdAndByLangId($countryId, $langId)
     {
         $query = $this->db->getQuery(true);
         $query->select('name')
             ->from($this->tableNames)
             ->where("`country_id` = {$countryId} AND `lang_id` = {$langId}")
             ->order('name ASC');
