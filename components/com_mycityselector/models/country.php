...
class MycityselectorModelCountry extends Joomla\CMS\MVC\Model\ListModel
{
    ...
    function __construct($config = [])
    {
        if (empty($config['filter_fields'])) {
            $config['filter_fields'] = ['ordering', 'id', 'name', 'subdomain', 'published'];
        }
     ...
     * @return array
     */
    public function getItems($limit = true, $enabledOnly = false)
    {
    ...
    protected function getListQuery()
    {
        ...
        $query->select('a.id as id, c.name as name, a.subdomain as subdomain, a.published as published, a.ordering as ordering')
            ->from($this->table . ' as a')
            ->innerJoin("`{$this->tableNames}` AS `c` ON `a`.`id` = `c`.`country_id`")
