...
/**
 * This function are help to read correct parameters of multiple modules on different languages of site
 * @return string Params
 */
function mod_readMyParams()
{
    ...
    $db = JFactory::getDbo();
    if ($id > 0) {
        $db->setQuery("SELECT * FROM `#__modules` WHERE `id`={$id}");
    } else {
        $db->setQuery("SELECT * FROM `#__modules` WHERE `module`='mod_mycityselector'");
    }
...
// start
mod_mycityselector(mod_readMyParams());