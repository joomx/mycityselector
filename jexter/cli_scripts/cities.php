...
foreach ($regions as $regionName => $region) {
    if (!empty($region)) {
        foreach ($region as $city) {
            $db->setQuery('SELECT id FROM #__mycityselector_region WHERE name='. $db->quote($regionName));
            $regionId = $db->loadRow()[0];
            $name = $db->escape(trim($city));
            out("  {$name}\n", 'light_blue');
            $db->setQuery("INSERT INTO #__mycityselector_city (`country_id`,`region_id`, `name`, `subdomain`) VALUES (1,'{$regionId}', '{$name}', '')");
            $db->execute();
        }
    }
}
