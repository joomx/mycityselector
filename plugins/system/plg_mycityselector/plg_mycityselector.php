...
class plgSystemPlg_Mycityselector extends JPlugin {
	...
	function __construct( &$subject, $params ){
		...
		global $MSC_BASE_DOMAIN; global $MSC_CURRENT_CITY;
	...
	function onAfterRender(){
		...
		if( $app->getName() == 'administrator' ){ // не делаем замену в админке
			if( JRequest::getVar('option') == 'com_modules' && JRequest::getVar('id') == $this->modID ){
			...
			if( $this->editMode ){  return;  } // не делаем замену в режиме редактирования статьи
